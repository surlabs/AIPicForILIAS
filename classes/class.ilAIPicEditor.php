...
class ilAIPicEditorGUI
{
    ...
    public function generateImage(?string $url = null): string
    {
        ...
        // Add language variables to DOM so Javascript can read them
        return '<div style="display: flex; align-items: center; flex-direction: column; justify-content: center;">' .
            ...
            // --- NUEVO: Inyectamos el mensaje del administrador aquí ---
    ...
    public function manageDownloadImage(): void
    {
        ...
        if ($request->getMethod() == "GET" && $actionFinal == "downloadImage" && $query->has("urlDownload")) {
            ...
            // Suppress warnings on expired URLs to prevent ILIAS header corruption
            ...
            $extension = pathinfo(parse_url($destiny, PHP_URL_PATH), PATHINFO_EXTENSION);
            $extension = $extension ?: 'png';

            $content_type = $params['rsct'] ?? 'image/' . $extension;
            $now = date_create()->format('Y-m-d_H-i-s');
            $fileName = "AIPic_$now.$extension";
    ...
    {
        // Prevent PHP thread timeout during slow AI responses
        set_time_limit(120);
        ...
        // --- START: CUSTOM ERROR HANDLER INTERCEPTOR ---
        // Retrieve the response generated by the Abstract class
        $rawAiResponse = $this->AIPicProvider->getResponse();
        $decodedResponse = json_decode($rawAiResponse, true);

        // Check if the abstract class intercepted an error and generated our friendly message
        if (is_array($decodedResponse) && isset($decodedResponse['error']) && $decodedResponse['error'] === true) {

            // Extract the language key set by the abstract class, fallback to generic if missing
            $langKey = $decodedResponse['error_key'] ?? 'no_images_found';

            // Translate the message using ILIAS plugin language variables dynamically
            $decodedResponse['message'] = $this->plugin->txt($langKey);

            // Output our custom JSON directly and halt execution
            echo json_encode($decodedResponse);
            exit();
        }
        // --- END: CUSTOM ERROR HANDLER INTERCEPTOR ---

        // Expose raw AI response to handle moderation filters or quota limits in frontend
        echo json_encode([
